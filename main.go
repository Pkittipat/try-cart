...
func main() {
	...
	productA := cart.Product{ID: "A", Price: decimal.NewFromFloat(100.00), Discount: 10} // 100.00 with 10% discount	
	productB := cart.Product{ID: "B", Price: decimal.NewFromFloat(200.00), Discount: 0}  // 200.00 no discount  	
	productC := cart.Product{ID: "C", Price: decimal.NewFromFloat(50.53), Discount: 15}  // 50.53 with 15% discount
	...
	fmt.Printf("- Product A x3: %s\n", cart.DisplayPrice(productA.GetDiscountedPrice().Mul(decimal.NewFromInt(3))))
	fmt.Printf("- Product B x1: %s\n", cart.DisplayPrice(productB.GetDiscountedPrice().Mul(decimal.NewFromInt(1))))
	fmt.Printf("- Product C x2: %s\n", cart.DisplayPrice(productC.GetDiscountedPrice().Mul(decimal.NewFromInt(2))))
	...
	invalidProduct := cart.Product{ID: "INVALID", Price: decimal.NewFromFloat(100.00), Discount: 150}
